    ...
    <div className="bg-white min-h-screen">
      ...
      <main className="container mx-auto px-6 py-12 md:py-24">
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <div className="mt-8 flex flex-wrap justify-center gap-3">
                {['LCK','LPL','LLA','VCJ','PMJL'].map((tag) => (
                  <span key={tag} className="px-4 py-2 rounded-full border border-purple-500/40 bg-purple-500/10 text-purple-600 text-sm font-semibold">
                    {tag}
                  </span>
                ))}
              </div>
        ...
        <section className="mb-24">
          <h2 className="text-4xl font-black text-gray-900 mb-12 text-center">코치 프로필</h2>
                ...
                <div className="bg-white rounded-2xl h-full w-full p-8">
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="text-2xl font-black text-gray-900">{coach.displayName}</h3>
                        <p className="text-purple-600 font-bold">{coach.title}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <div>
                      <h4 className="text-base font-bold text-gray-900 mb-3">선수</h4>
                      <ul className="text-gray-700 space-y-2 text-sm">
                        {coach.playerHistory.map((h) => (
                          <li key={`${coach.id}-player-${h.year}-${h.team}`} className="flex items-center gap-3">
                            <span className="text-purple-600 min-w-[56px]">{h.year}</span>
                    ...
                    <div>
                      <h4 className="text-base font-bold text-gray-900 mb-3">코치</h4>
                      <div className="relative">
                        <div className="absolute left-2 top-0 bottom-0 w-px bg-purple-500/30"></div>
                        <ul className="text-gray-700 space-y-3 text-sm pl-6">
